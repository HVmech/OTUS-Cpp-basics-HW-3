...
int main(int argc, char** argv)
{
    ...
    std::ofstream ofile{high_scores_filename, std::ios::out | std::ios::app};
    if (!ofile.is_open())
    {
        ofile.open(high_scores_filename, std::ios::out | std::ios::app);
        
	if (!ofile.is_open())
	{
	    std::cout << "Failed attempt to open/create file: " << high_scores_filename << "!" << std::endl;
	    return -1;
        }
